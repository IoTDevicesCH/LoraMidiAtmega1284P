...
// As an addition, in this sketch the MCU goes into idle state for multiples of 1ms to save power while waiting for the RFM95 module to complete TX or wait for RX window.
// This saves about 60% of power while waiting instead of letting the MCU simply powered on in normal mode.
...
void loop() {
    ...
    if (next == false) {
        ...
        // Instead we can put the MCU into idle state with TIMER0_ON for whatever time interval (SLEEP_8S used below), 
        // as TIMER0 will overflow every 1ms, MCU will wake up every ms again. We can save about 60% of the power during
        // waiting with this simple trick.
        ...
        if (os_queryTimeCriticalJobs(ms2osticks(1)) == 0) {
            ...
            LowPower.idle(SLEEP_8S, ADC_OFF, TIMER2_OFF, TIMER1_OFF, TIMER0_ON, SPI_OFF, USART1_OFF, USART0_OFF, TWI_OFF);
        }
    }
    else {
